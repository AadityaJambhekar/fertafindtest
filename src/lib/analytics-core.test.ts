...
test("detects a ChatGPT referrer URL", () => {
  ...
  assert.equal(detectAiReferral("https://chat.openai.com/c/abc", null), "chatgpt");
});
...
test("detects Perplexity, Gemini, Claude and Copilot referrers", () => {
  assert.equal(detectAiReferral("https://www.perplexity.ai/search", null), "perplexity");
  assert.equal(detectAiReferral("https://gemini.google.com/app", null), "gemini");
  assert.equal(detectAiReferral("https://claude.ai/chat/1", null), "claude");
  assert.equal(detectAiReferral("https://copilot.microsoft.com/", null), "copilot");
});

test("returns null for ordinary search and social referrers", () => {
  assert.equal(detectAiReferral("https://www.google.com/search?q=fertilizer", null), null);

...
export type PartnerStageAvailability = "available" | "needs-verification" | "unavailable";

export function getPartnerStageAvailability(crop: string, stage: string): PartnerStageAvailability {
  const matchingUses = partnerProducts.flatMap((product) =>
    product.uses.filter((use) => useMatchesCrop(use, crop)),
  );
  if (!matchingUses.length) return "unavailable";

  const stageStatuses = matchingUses.map((use) => stageMatch(use, stage));
  if (stageStatuses.includes("compatible")) return "available";
  if (stageStatuses.includes("unknown")) return "needs-verification";
  return "unavailable";
}

function priorNutrientCredits(analysis: QuoteAnalysis) {
  const credits: Record<"N" | "P" | "K", number> = { N: 0, P: 0, K: 0 };
  for (const application of analysis.farmContext?.priorFertilizerApplications ?? []) {
    const analysisMatch = application.productAnalysis.match(
      /(?:^|\s)(\d+(?:\.\d+)?)\s*[-–]\s*(\d+(?:\.\d+)?)\s*[-–]\s*(\d+(?:\.\d+)?)(?:\s|$)/,
    );
    const rawQuantity = Number(application.quantity);
    if (!analysisMatch || !Number.isFinite(rawQuantity) || rawQuantity <= 0) continue;
    const quantityKgHa =
      application.unit === "kg/ha"
        ? rawQuantity
        : application.unit === "lb/ac"
          ? rawQuantity * 1.120851
          : null;
    if (!quantityKgHa) continue;
    const percentages = analysisMatch.slice(1, 4).map(Number);
    credits.N += quantityKgHa * (percentages[0] / 100);
    credits.P += quantityKgHa * (percentages[1] / 100);
    credits.K += quantityKgHa * (percentages[2] / 100);
  }
  return credits;
}
...
function targetNutrients(analysis: QuoteAnalysis) {
  ...
  const pairs: Array<[NutrientCode, string]> = [
    ...
    ["S", analysis.nutrientPlan?.targetSulfurKgHa],
  ];
  const priorCredits = priorNutrientCredits(analysis);
  return pairs.flatMap(([nutrient, value]) => {
    const parsed = Number(value);
    const credited =
      nutrient === "N" || nutrient === "P" || nutrient === "K" ? priorCredits[nutrient] : 0;
    return Number.isFinite(parsed) && parsed > credited ? [nutrient] : [];
  });
}
...
function hasLaboratorySoil(analysis: QuoteAnalysis) {
  ...
  return Boolean(
    ...
    [
      ...
      context.soilCec,
      context.soilTexture,
      ...Object.values(context.soilMicronutrients ?? {}),
...
function evaluateProduct(
  ...
  if (matchingTargets.length) {
    ...
    factors.push({
      label: "Nutrient priorities",
      status: "missing",
      detail: "No nutrient priority or positive target N, P, K or S rate was supplied.",
    });
    missingInformation.push("Target N-P-K-S plan");
  }
  ...
  const priorApplications = analysis.farmContext?.priorFertilizerApplications ?? [];
  if (analysis.farmContext?.priorFertilizerApplied) {
    const completeApplications = priorApplications.filter(
      (application) =>
        application.productAnalysis.trim() && application.quantity.trim() && application.unit,
    );
    const credits = priorNutrientCredits(analysis);
    const creditSummary = (Object.entries(credits) as Array<["N" | "P" | "K", number]>)
      .filter(([, amount]) => amount > 0)
      .map(([nutrient, amount]) => `${amount.toFixed(1)} kg/ha ${nutrient}`)
      .join(", ");
    factors.push({
      label: "Prior fertilizer",
      status: completeApplications.length === priorApplications.length ? "used" : "caution",
      detail: completeApplications.length
        ? creditSummary
          ? `${completeApplications.length} prior application${completeApplications.length === 1 ? " was" : "s were"} included; explicit N-P-K analysis produced a provisional credit of ${creditSummary}.`
          : `${completeApplications.length} prior application${completeApplications.length === 1 ? " was" : "s were"} included, but no nutrient credit was calculated without an explicit N-P-K analysis in kg/ha or lb/ac.`
        : "Prior fertilizer was reported, but product and quantity details were incomplete.",
    });
    if (completeApplications.length !== priorApplications.length) {
      missingInformation.push("Complete prior-application product and quantity details");
    }
  }
...
export function recommendPartnerProducts(analysis: QuoteAnalysis): CropPartnerRecommendation[] {
  return parseCropSelections(analysis.crop, analysis.farmContext?.cropStage ?? "").map(
    ({ crop, stage }) => {
      if (analysis.preferences?.organicCertification === "certified") {
        const hasVerifiedOmriProduct = partnerProducts.some((product) => product.omriApproved);
        if (!hasVerifiedOmriProduct) {
          return {
            crop,
            ...
            summary:
              "This field is certified organic, but no participating partner product currently has verified OMRI approval in the catalog. No unverified product is being recommended.",
            selectedProducts: [],
            alternatives: [],
            excluded: [],
            factorsUsed: [
              {
                ...
                status: "caution",
                detail: "Verified OMRI approval is required and is currently unavailable.",
              },
            ],
            missingInformation: ["Verified OMRI approval or permitted-input documentation"],
      ...
      const cropUses = partnerProducts.flatMap((product) =>
        product.uses
          .filter((use) => useMatchesCrop(use, crop))
          .filter(
            () =>
              analysis.preferences?.organicCertification !== "certified" ||
              product.omriApproved === true,
          )
          .map((use) => ({ product, use })),
      );

...
test("every public page has a unique, non-empty description", () => {
  ...
  for (const d of descriptions) assert.ok(d && d.trim().length > 0, "description present");
  assert.equal(new Set(descriptions).size, descriptions.length, "descriptions are unique");
});
...
test("pageMeta gives each page its own Open Graph title, description and url", () => {
  for (const key of PAGE_KEYS) {
    ...
    const byProp = (p: string) =>
      meta.find((m) => "property" in m && m.property === p) as { content?: string } | undefined;
    const byName = (n: string) =>
      meta.find((m) => "name" in m && m.name === n) as { content?: string } | undefined;
    ...
    assert.ok(byName("twitter:description")?.content, `${key} has twitter:description`);
...
test("no page inherits the homepage Open Graph copy", () => {
  const ogTitle = (key: PageKey) =>
    (
      pageMeta(key).meta.find((m) => "property" in m && m.property === "og:title") as {
        content?: string;
      }
    ).content;
  ...
  for (const key of PAGE_KEYS) {
    if (key === "home") continue;
    assert.notEqual(ogTitle(key), home, `${key} og:title differs from homepage`);
  }
});

...
function stringRecordField(form: FormData, name: string) {
  try {
    const parsed = JSON.parse(field(form, name)) as unknown;
...
function priorApplicationsField(form: FormData): PriorFertilizerApplication[] {
  try {
    const parsed = JSON.parse(field(form, "priorFertilizerApplications")) as unknown;
    if (!Array.isArray(parsed)) return [];
    return parsed.flatMap((entry) => {
      if (!entry || typeof entry !== "object") return [];
      const item = entry as Record<string, unknown>;
      const unit = item.unit;
      if (!(["kg/ha", "L/ha", "lb/ac", "gal/ac"] as unknown[]).includes(unit)) return [];
      return [
        {
          applicationDate:
            typeof item.applicationDate === "string" ? item.applicationDate.trim() : "",
          productAnalysis:
            typeof item.productAnalysis === "string" ? item.productAnalysis.trim() : "",
          quantity: typeof item.quantity === "string" ? item.quantity.trim() : "",
          unit: unit as PriorFertilizerApplication["unit"],
        },
      ];
    });
  } catch {
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const soilSulfur = field(form, "soilSulfur");
        ...
        const soilMicronutrients = stringRecordField(form, "soilMicronutrients");
        const priorFertilizerApplied = field(form, "priorFertilizerApplied");
        const priorFertilizerApplications = priorApplicationsField(form);
        ...
        const targetSulfurKgHa = field(form, "targetSulfurKgHa");
        ...
        if (
          ...
          (fertilizerOriginPreference === "organic" &&
            !["certified", "not_certified"].includes(organicCertification)) ||
...
Organic certification status: ${fertilizerOriginPreference === "organic" ? (organicCertification === "certified" ? "certified organic; treat verified OMRI approval as a hard requirement" : "not certified; organic inputs are a preference only") : "not applicable"}. For a certified-organic farm, never mark a product suitable or recommended unless the uploaded product evidence explicitly verifies OMRI approval or equivalent permitted-input status. Unknown status is not approval.
...
- Soil test results (preserve the grower's stated units): nitrate-N ${soilNitrogen || "not supplied"}, available P ${soilPhosphorus || "not supplied"}, exchangeable K ${soilPotassium || "not supplied"}, sulfate-S ${soilSulfur || "not supplied"}, pH ${soilPh || "not supplied"}, organic matter ${soilOrganicMatter || "not supplied"}, CEC ${soilCec || "not supplied"}, texture ${soilTexture || "not supplied"}; micronutrients ${
          Object.entries(soilMicronutrients)
            .filter(([, value]) => value)
...
- Prior fertilizer applications this season: ${priorFertilizerApplied === "no" ? "grower reported none" : priorFertilizerApplied === "yes" ? (priorFertilizerApplications.length ? priorFertilizerApplications.map((application) => `${application.applicationDate || "date not supplied"}: ${application.productAnalysis || "product not supplied"}, ${application.quantity || "quantity not supplied"} ${application.unit}`).join("; ") : "grower reported prior fertilizer, but supplied no application details") : "not supplied"}. Account for a prior nutrient contribution only when the product analysis, quantity and unit make that contribution explicit. Otherwise flag it for verification instead of subtracting an assumed amount.
- Target nutrient plan: N ${targetNitrogenKgHa || "not supplied"} kg/ha, P ${targetPhosphorusKgHa || "not supplied"} kg/ha, K ${targetPotassiumKgHa || "not supplied"} kg/ha, S ${targetSulfurKgHa || "not supplied"} kg/ha
...
Extract every distinct fertilizer product offered or listed in the uploaded files. A valid quote item includes a line from a quotation, estimate, invoice, order form, price list, supplier proposal, product sheet, spreadsheet, email, screenshot, or handwritten offer. Do not require a price, N-P-K analysis, pack size, application rate, supplier name, or delivery charge before including an identifiable fertilizer product. If a row visibly identifies a fertilizer by product name or grade/analysis, include it and leave every genuinely missing field blank or null. Only return an empty quotes array when none of the supplied pages contains any identifiable fertilizer product or fertilizer grade.

Copy only values visible in the files. Do not invent a supplier, product name, price, delivery charge, application rate, currency, pack size, nutrient value, or soil requirement. Use null for a missing numeric value, an empty string for missing text, and [0,0,0] only when N-P-K is not stated. Convert tonnes to kilograms where necessary. pricePerBag means the price for the stated pack/bag/bulk unit whose mass is bagKg. deliveryPerT means delivery cost per metric tonne. applicationRateKgHa means kilograms per hectare. Put uncertainty, illegible text, missing commercial fields, or unit conversions in notes. Inspect every supplied page and every visible table row before concluding that no product was found.
          ...
          const analysis: QuoteAnalysis = {
            ...
            preferences: {
              ...
              organicCertification:
                fertilizerOriginPreference === "organic"
                  ? (organicCertification as "certified" | "not_certified")
                  : null,
            },
            fieldSize,
            unit,

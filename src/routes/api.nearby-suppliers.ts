...
type Supplier = {
  ...
  website: string | null;
  phone: string | null;
  supplier_products: Product[] | null;
};
type OSMElement = {
  id: number;
  type: string;
...
function supplierDistance(value: unknown) {
  if (!value || typeof value !== "object" || !("distanceKm" in value)) return Infinity;
  const distance = (value as { distanceKm?: unknown }).distanceKm;
  return typeof distance === "number" && Number.isFinite(distance) ? distance : Infinity;
}
    ...
    return [
      {
        ...
        latitude: point.lat,
        longitude: point.lon,
        distanceKm: Math.round(distanceKm(lat, lon, point.lat, point.lon) * 10) / 10,
  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        const farmerRadiusKm = Math.min(
          Math.max(Number(url.searchParams.get("radiusKm")) || 50, 1),
          1000,
        );
        if (!Number.isFinite(lat) || !Number.isFinite(lon))
          return Response.json({ error: "A farm location is required." }, { status: 400 });
        ...
        if (supabaseUrl && publicKey) {
          ...
          endpoint.searchParams.set(
            "select",
            "id,business_name,address,latitude,longitude,delivery_radius_km,description,website,phone,supplier_products(id,product_name,nitrogen_percent,phosphorus_percent,potassium_percent,package_kg,price_per_unit,delivery_per_tonne,currency,active)",
          );
          ...
          if (response.ok) {
            ...
            listedSuppliers = suppliers
              .map((supplier) => ({
                ...supplier,
                distanceKm:
                  Math.round(distanceKm(lat, lon, supplier.latitude, supplier.longitude) * 10) / 10,
                products: (supplier.supplier_products ?? []).filter((product) => product.active),
              }))
              .filter(
                (supplier) =>
                  supplier.distanceKm <= Math.min(farmerRadiusKm, supplier.delivery_radius_km),
              )
              .map((supplier) => ({ ...supplier, source: "FertaFind" }));
          }
        }
        const mappedSuppliers = await discoverOpenStreetMapSuppliers(
          lat,
          lon,
          farmerRadiusKm,
        ).catch(() => []);
        const nearby = [...listedSuppliers, ...mappedSuppliers]
          .sort((a, b) => supplierDistance(a) - supplierDistance(b))
          .slice(0, 10);
        return Response.json(
          { suppliers: nearby, openStreetMapAttribution: mappedSuppliers.length > 0 },
